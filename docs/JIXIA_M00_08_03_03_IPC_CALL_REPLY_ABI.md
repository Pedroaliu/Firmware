...
- `server` TaskId — the TaskId of the task whose recv/try_recv delivered the
  request, captured by the kernel at delivery;
...
- **Endpoint ABA:** destroy + recreate advances the endpoint generation; any
  ...
  stale endpoint handles in .03.01).
- **Transaction ABA:** every consume or invalidation advances the slot's
  ...
  mismatches → `-EINVAL`.
- **Generation wrap:** the transaction generation caps at `0xFFFFFF`; at the
  cap the slot retires **for the remainder of the current endpoint epoch**
  (mirroring endpoint slot retirement at `kMaxGeneration`). A fresh endpoint
  epoch (destroy + recreate) starts a fresh transaction table, because every
  old token is already invalid by epoch mismatch.
...
2. liveness: endpoint epoch matches, slot not retired, transaction generation
   matches, state == `DELIVERED_WAITING_REPLY` → else `-EINVAL` (stale);
3. server binding: bound server TaskId == caller TaskId → else `-EACCES`
...
`{state, generation, caller Task*, server TaskId, request[4]}` — static,
guarded by the endpoint's existing lock. States: `FREE`, `REQUEST_PENDING`,
`DELIVERED_WAITING_REPLY`, `ANSWERED`, `DEAD`.
...
```text
  ...
  REQUEST_PENDING ── receiver pops request ──▶ DELIVERED_WAITING_REPLY → ANSWERED → FREE
    │                                                (slot generation++ each cycle)
    └──────────── destroy (any live state) ──────────────────────────┘
                caller woken -EIDRM exactly once, token invalidated
...
| FREE → DELIVERED_WAITING_REPLY | `ipc_call`: waiter present, slot free | waiter's registers written (`a0=0`, `a1..a4` request, `a5` caller TaskId, `a6` token), waiter published READY via `add_task`; server binding = waiter TaskId; waiter's `reply_obligation_count++` |
| REQUEST_PENDING → DELIVERED_WAITING_REPLY | recv/try_recv pops the request | server binding = popping task's TaskId; that task's `reply_obligation_count++` |
| DELIVERED_WAITING_REPLY → ANSWERED → FREE | `ipc_reply` consume | reply words written into the caller's saved registers (`a0=0`, `a1..a4` reply, `a5` replier TaskId), caller's `call_wait_token` cleared, caller state → ready, `add_task`; slot generation++; server's `reply_obligation_count--` |
| REQUEST_PENDING / DELIVERED_WAITING_REPLY → DEAD → FREE | endpoint destroy | caller woken with `a0 = -EIDRM` exactly once, `call_wait_token` cleared, caller READY; token invalid via epoch mismatch; delivered transactions decrement the bound server's `reply_obligation_count` |
...
- **Waiting receiver present:** identical to send's fast path — pop exactly one
  waiter, write its return registers (with `a5` = caller TaskId and `a6` =
  token), `add_task` inside the lock. No handoff, no forced yield.
...
1. `active = false`, endpoint generation++ (existing .03.01 linearization);
...
4. all unconsumed tokens of this endpoint are stale by epoch mismatch (old
   endpoint handles keep their existing `-EINVAL` behavior);
5. for each transaction that had been delivered, decrement the bound server's
   `reply_obligation_count`.
...
|---|---|---|
| `ipc_call` direct delivery | waiter popped + registers written + `add_task` | endpoint |
...
- A `Task` is in at most one of: the runqueue, one endpoint waiting FIFO, one
  live call transaction. Guards: `RunQueue::insert`/`remove`,
  `Scheduler::add_task`, `TaskManager::set_current_task`, and
  `TaskManager::release_task_locked` all refuse (fail closed) a task violating
  membership — the call-wait marker is `call_wait_token != 0` (with
  `TaskState::blocked_message`), not a new state value.
...
- `task_end` / `release_task_locked` refuse (fail closed — the same discipline
  as the .03.02 `message_wait.queued` refusal: no silent leak, no silent UAF)
  when: `message_wait.queued` (existing), `call_wait_token != 0` (caller of a
  live transaction), or `reply_obligation_count != 0` (server holding
  unconsumed tokens).
- Benign race, frozen as safe: a release-path load may observe a nonzero count
  that a concurrent destroy is about to clear; refusal is always the safe
  answer and the exit can be re-attempted after the destroy linearizes. The
  reverse (observing 0 while a delivery is in flight) cannot happen: the
  increment precedes the token becoming visible to the server, in the same
  critical section as the delivery.
- **Deliberately deferred to M00-08.03.04:** revoking outstanding tokens on
  ...
  timeouts, and any non-fail-closed cleanup path. M00-08.03.03 ships only the
  constraints above.
...
|---|---|---|---|---|
| endpoint slot + epoch | creator task | `endpoint_create` | `endpoint_destroy` (owner-only) | epoch advances; handles stale |
| FIFO message entry | endpoint | send / call enqueue | recv / try_recv pop | entries dropped with the epoch |
| transaction slot + token | kernel (endpoint-scoped) | call (one critical section) | reply consume or destroy invalidation; slot retires at generation cap | all live transactions DEAD; callers woken `-EIDRM`; tokens stale |
...
| user ReplyToken | receiving server | delivery (`a6`) | a single `ipc_reply` | invalid (epoch mismatch); obligation decremented |
...
- All .03.02 invariants survive (§4); the membership refusal set grows by
  `call_wait_token` / `reply_obligation_count` only.
...
| `microkernel/core/task_manager.cpp` | release/exit refusals; task-create initialization |
...
| C22 wrong server | `WRONG_SERVER_EACCES` (token still live; a later correct reply may succeed) |
