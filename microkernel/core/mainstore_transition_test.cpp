...
extern "C" [[noreturn]] void jixia_m00_07_04_run_mainstore_transition_probe() {
    ...
    *stable_object = kStableObjectMagic;
    ...
    const PhysicalRange future_remaining = {
        .base = future_ddr.base + initial_state.contained.size,
        .size = future_ddr.size - initial_state.contained.size,
    };
    ...
    const Allocation gated_page = memory::page_manager::allocate_page();
    if (gated_page.valid()) {
        fail("allocation escaped mainstore gate");
    }
    printk("M00_07_MAINSTORE_ALLOCATOR_GATED: PASS\n");

    if (!memory::enable_mainstore_allocation() || !memory::validate_mainstore_invariants()) {
