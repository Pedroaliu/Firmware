...
bool validate_contained_invariants()
{
    ...
    if (g_state.contained.base != expected_base || g_state.contained.size != kQemuContainedWindowSize)
    {
        return false;
    }
    ...
    const uintptr_t first_address_after_contained =
        g_state.contained.base + g_state.contained.size;
