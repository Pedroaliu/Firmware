...
readonly BUILD_PATH="${JIXIA_M00_08_01_BUILD_DIR:-${ROOT_DIR}/build/m00-08-01}"
readonly TIMEOUT_SECONDS="${JIXIA_QEMU_TIMEOUT_SECONDS:-5}"
readonly SERIAL_LOG="${BUILD_PATH}/m00-08-01.serial.log"
readonly LOG_FILE="${BUILD_PATH}/m00-08-01.log"
readonly QEMU_ERROR_LOG="${BUILD_PATH}/m00-08-01.qemu.log"
...
if [[ ${QEMU_STATUS} -ne 124 ]]; then
    ...
    echo "M00-08.01: unexpected QEMU status ${QEMU_STATUS}" >&2
    exit 1
fi
...
do
    if grep -Fq "${forbidden_marker}" "${LOG_FILE}"; then
        echo "M00-08.01: forbidden marker observed: ${forbidden_marker}" >&2
        exit 1
    fi
done
...
for required_marker in \
    ...
    "M00_08_TASK_END: PASS" \
    "M00_08_TASK_WAIT: PASS" \
    "M00_08_TASK_WAIT_BLOCK: PASS" \
    "M00_08_TASK_DETACH: PASS" \
    "M00_08_TASK_LIFECYCLE: PASS" \
    "M00_08_IDLE_TASK: PASS"
do
    if ! grep -Fxq "${required_marker}" "${LOG_FILE}"; then
        echo "M00-08.01: required marker not found: ${required_marker}" >&2
        exit 1
    fi
done

echo "M00-08.01 Hostboot-shaped task lifecycle: PASS"
