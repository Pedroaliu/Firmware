...
readonly SERIAL_LOG="${BUILD_PATH}/console-test.serial.log"
readonly LOG_FILE="${BUILD_PATH}/console-test.log"
readonly QEMU_ERROR_LOG="${BUILD_PATH}/console-test.qemu.log"
...
if [[ ${QEMU_STATUS} -ne 124 ]]; then
    ...
    echo "Console test: unexpected QEMU status ${QEMU_STATUS}" >&2
    exit 1
fi

if grep -Fq "[Jixia][Microkernel][fatal trap]" "${LOG_FILE}"; then
    echo "Console test: fatal trap observed" >&2
    exit 1
fi

if grep -Fq "CONSOLE_TEST: FAIL" "${LOG_FILE}"; then
    echo "Console test: FAIL" >&2
    exit 1
fi

if ! grep -Fxq "console-memory-probe" "${LOG_FILE}"; then
    echo "Console test: serial sink probe marker not found" >&2
    exit 1
fi

if ! grep -Fxq "memory     : ring sink retained exact probe" "${LOG_FILE}"; then
    echo "Console test: memory sink validation marker not found" >&2
    exit 1
fi

if ! grep -Fxq "CONSOLE_TEST: PASS" "${LOG_FILE}"; then
    echo "Console test: PASS marker not found" >&2
    exit 1
fi
