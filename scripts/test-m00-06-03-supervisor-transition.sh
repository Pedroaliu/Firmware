...
readonly BUILD_PATH="${JIXIA_M00_06_03_BUILD_DIR:-${ROOT_DIR}/build/m00-06-03}"
readonly TIMEOUT_SECONDS="${JIXIA_QEMU_TIMEOUT_SECONDS:-5}"
readonly SERIAL_LOG="${BUILD_PATH}/m00-06-03.serial.log"
readonly LOG_FILE="${BUILD_PATH}/m00-06-03.log"
readonly QEMU_ERROR_LOG="${BUILD_PATH}/m00-06-03.qemu.log"
...
if [[ ${QEMU_STATUS} -ne 124 ]]; then
    ...
    echo "M00-06.03: unexpected QEMU status ${QEMU_STATUS}" >&2
    exit 1
fi

if grep -Fq "[Jixia][Microkernel][fatal trap]" "${LOG_FILE}"; then
    echo "M00-06.03: fatal trap observed" >&2
    exit 1
fi

if grep -Fq "M00_06_03_SUPERVISOR_ECALL_RETURN: FAIL" "${LOG_FILE}"; then
    echo "M00-06.03: supervisor return-side validation failed" >&2
    exit 1
fi
...
do
    if ! grep -Fxq "${required_marker}" "${LOG_FILE}"; then
        echo "M00-06.03: required marker not found: ${required_marker}" >&2
        exit 1
    fi
done

echo "M00-06.03 supervisor ECALL round trip: PASS"
