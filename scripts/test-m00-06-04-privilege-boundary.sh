...
readonly BUILD_PATH="${JIXIA_M00_06_04_BUILD_DIR:-${ROOT_DIR}/build/m00-06-04}"
readonly TIMEOUT_SECONDS="${JIXIA_QEMU_TIMEOUT_SECONDS:-5}"
readonly SERIAL_LOG="${BUILD_PATH}/m00-06-04.serial.log"
readonly LOG_FILE="${BUILD_PATH}/m00-06-04.log"
readonly QEMU_ERROR_LOG="${BUILD_PATH}/m00-06-04.qemu.log"
...
if [[ ${QEMU_STATUS} -ne 124 ]]; then
    ...
    echo "M00-06.04: unexpected QEMU status ${QEMU_STATUS}" >&2
    exit 1
fi

if grep -Fq "[Jixia][Microkernel][fatal trap]" "${LOG_FILE}"; then
    echo "M00-06.04: fatal trap observed" >&2
    exit 1
fi

for forbidden_marker in \
    "M00_06_04_HOSTILE_SP_RETURN: FAIL" \
    "M00_06_04_NO_ANCHOR_RETURN: FAIL"
do
    if grep -Fq "${forbidden_marker}" "${LOG_FILE}"; then
        echo "M00-06.04: forbidden marker observed: ${forbidden_marker}" >&2
        exit 1
    fi
done
...
do
    if ! grep -Fxq "${required_marker}" "${LOG_FILE}"; then
        echo "M00-06.04: required marker not found: ${required_marker}" >&2
        exit 1
    fi
done

echo "M00-06.04 hostile lower-privilege boundary: PASS"
